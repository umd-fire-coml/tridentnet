...
class TridentFasterRcnn(object):#detector
    _rpn_output = None
    ...
    def get_test_symbol(cls, backbone, neck, rpn_head, roi_extractor, bbox_head, num_branch):
        ...
        roi_feat = roi_extractor.get_roi_feature(rcnn_feat, proposal)
        cls_score, bbox_xyxy,one,two,three = bbox_head.get_prediction(roi_feat, im_info_branches, proposal)
...
class TridentAlpha(object):#alphahead
    _rpn_output = None
    ...
    def get_train_symbol(cls, backbone, neck, rpn_head, roi_extractor, bbox_head, num_branch, scaleaware):
        ...
        if scaleaware:
            valid_ranges = X.var("valid_ranges")
        rpn_alpha_label = X.var("rpn_alpha_label")
        rpn_reg_target = X.var("rpn_reg_target")
        rpn_reg_weight = X.var("rpn_reg_weight")
...
class TridentRpnHead(RpnHead):#rpnhead
...
class TridentDimHead(RpnHead):#rpnhead
    def __init__(self, pRpn):
        super().__init__(pRpn)
...
class TridentMXNetResNetV2(Backbone):#backboneS
